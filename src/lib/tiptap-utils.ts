...
export function focusNextNode(
  ...
  const nextSel = Selection.findFrom(
    selection.$to,
    1,
    true,
  );
  ...
  if (!paragraphType) {
    console.warn(
      "No paragraph node type found in schema.",
    );
    return false;
  }
...
export function findNodeAtPosition(
  ...
  try {
    ...
    console.error(
      `Error getting node at position ${position}:`,
      error,
    );
    return null;
  }
...
export function sanitizeUrl(
  ...
  try {
    ...
    if (
      isAllowedUri(url.href, protocols)
    ) {
      return url.href;
    }
  } catch {
...
>(
  ...
  for (const { pos } of targets) {
    ...
    const prevValue = (
      currentNode.attrs as Record<
        string,
        unknown
      >
    )[attrName] as V | undefined;
    const resolvedNext =
...
export function selectCurrentBlockContent(
  ...
  if (blockNode && blockPos >= 0) {
    const from = blockPos;
    const to =
      blockPos + blockNode.nodeSize - 2; // -2 to exclude the closing tag
...
export function getSelectedNodesOfType(
  ...
  if (cell) {
    const cellNode =
      selection.$anchor.doc.nodeAt(
        cell.pos,
      );
    if (
      cellNode &&
      allowed.has(cellNode.type.name)
    ) {
      results.push({
        node: cellNode,
        pos: cell.pos,
